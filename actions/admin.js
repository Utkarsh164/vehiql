...
export async function getAdminTestDrives({ search = "", status = "" }) {
  try {
      ...
      where.OR = [
        {
          car: {
            OR: [
              { make: { constains: search, mode: "insensitive" } },
              { model: { constains: search, mode: "insensitive" } },
            ],
          },
        },
        {
    ...
    console.log("Error fetching test drives:", error);
    return {
      success: false,
      error: error.message,
    };
...
export async function updateTestDriveStatus(bookingId, newStatus) {
  try {
    ...
    if (!booking) {
      throw new Error("Booking ot found");
    }
    const validStatus = [
      "PENDING",
      "CONFIRMED",
    ...
    throw new Error("Error updating the status:"+error.message);
  }
}
